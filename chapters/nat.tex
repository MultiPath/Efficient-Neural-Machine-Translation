...
% \begin{equation}
% \label{latent}
% p_{\mathcal{NA}}(Y|X; \theta) = \sum_z{\left[p_z(z|x_{1:T'};\theta)\cdot p_L(T|x_{1:T'};\theta;z)\cdot \prod_{t=1}^T p(y_t| x_{1:T'};\theta;z)\right]}.
% \end{equation}
...
%\subsection{Latent Fertility Model}
...
% Use teacher scores to fine-tune fertility predictions using REINFORCE
% If we don't use reinforcement learning, then our latent fertility model is actually just a supervised fertility model.
\section{Experiments}
...
% \bottomrule
% \end{tabular} 
% \caption{\label{table.dataset} Dataset statistics.}
% \end{wraptable}   
\vspace{-5pt}
...
\vspace{-10pt}
\centering
\includegraphics[width=\linewidth]{figs/nat/NAT-sample-bleu}
